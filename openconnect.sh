#!/bin/sh
#
# Davit Pivazyan, TopSoft Solution Center 2023 
# Referance: github.com/aw1cks
#
...
if [ -z "${VPN_SERVER}" ]
then
  printf "\e[31m\$VPN_SERVER is not set\n\e[0m" 
  exit 1
fi
printf "\e[33mVPN_SERVER:\e[0m %s \n" "${VPN_SERVER}"
...
# Set user
OPENCONNECT_ARGS="--background -i tun127 --passwd-on-stdin --non-inter"
...
# VPN_SERVER needs to be the last argument
printf "\e[32mSetting VPN_SERVER...\e[0m\n"

printf "\e[32mStarting OpenConnect VPN...\e[0m\n"
printf "\e[33mArguments:\e[0m %s\n\n" "${OPENCONNECT_ARGS}"
# shellcheck disable=SC2086
(echo "${PASS}"; [ -n "${OTP}" ] && echo "${OTP}") | openconnect ${OPENCONNECT_ARGS} --user="${USER}" ${VPN_SERVER}
